        ...
        if let Event::Key(key) = event::read()? {
            ...
            match app.current_screen {
                ...
                CurrentScreen::Game => match key.code {
                    KeyCode::Char('q') => return Ok(()),
                    KeyCode::Up => {
                        if !app.board.as_ref().unwrap().is_game_over() {
                    ...
                    KeyCode::Enter => {
                        ...
                        if let Some((row, col)) = app.selected_cell.as_ref() {
                                    ...
                                    match board.get_player_turn() {
                                        Cell::Black => {
                                            if let Some(player) = &app.player_1 {
                                                play_turn_result =
                                                    player.play_turn(board, app.selected_cell);
                                            }
                                        }
                                        Cell::White => {
                                        ...
                                        _ => {
                                            play_turn_result =
                                                Err("Invalid player turn".to_string());
                                        }
                                    }
                                        ...
                                        Ok(history_action) => {
                                            ...
                                            if board.check_game_over() {
                                                if let Some(winner) = board.get_winner() {
                                                    ...
                                                    new_message =
                                                        Some("Game over! It's a draw!".to_string());
                                                }
                                            } else {
                                                board.next_turn();
                                                new_message = Some(format!(
